...
function register() {
    ...
    if (!validate(Name.value, email.value, userName.value, address.value, password.value, passCheck.value)) { return; }
    const dbRef = ref(db);
    get(child(dbRef, "organization/" + userName.value)).then((snapshot) => {
        console.log(snapshot)
        if (snapshot.exists()) {
            callAlert("Account already exists!", "red");
        } else {
            console.log(snapshot)
            set(ref(db, "organization/" + userName.value), {
