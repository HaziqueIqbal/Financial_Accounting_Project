...
function getFromFireBase1() {
    ...
    get(child(dbRef, `inventory_management/${gets.Username}`)).then((snapshot) => {
        if (snapshot.exists()) {
            if (snapshot.val().ledgers.three != undefined) {
                extra.push(snapshot.val().ledgers.three.AllEnteries)
            }
    ...
    if (getType == "discount") {
        for (let index = 0; index < getInfoDB[0].length; index++) {
            if (name.value.toLowerCase() == getInfoDB[0][index].AccountName) {
                ...
                all_enteries.push({ "AccountName": "Accounts Payable", "Debit": finalvalue, "Credit": 0, "typeOfAccount": "liability", "v": 0, "date": current });
                all_enteries.push({ "AccountName": name.value.toLowerCase(), "Debit": 0, "Credit": y, "typeOfAccount": "asset", "v": 1 });
                all_enteries.push({ "AccountName": "cash", "Debit": 0, "Credit": z, "typeOfAccount": "asset", "v": 2 });
                ...
                if (x != 0) {
                    for (let m = 0; m < getInfoDB[0].length; m++) {
                        ...
                        if (getInfoDB[0][m].AccountName == "cash") {
                            getInfoDB[0][m].Credit += y;
                            break;
                        } else {
                            getInfoDB[0].push({ "AccountName": "cash", "Debit": 0, "Credit": z, "typeOfAccount": "asset" });
                            break;
                        }
                ...
                getInfoDB[0][index].Credit += y;
            }
        }
        for (let index = 0; index < getInfoDB[0].length; index++) {
            if (getInfoDB[0][index].AccountName == "accounts payable") {
                getInfoDB[0][index].Debit += finalvalue;
            }
        }
        // console.log(getInfoDB[0])
    }
...
function saveToFirebase(getUser) {
    ...
    get(child(dbRef, `inventory_management/${getUser.Username}`)).then((snapshot) => {
        // console.log("true")
        if (snapshot.exists()) {
            set(ref(db, "inventory_management/" + getUser.Username + `/ledgers/three`), {
