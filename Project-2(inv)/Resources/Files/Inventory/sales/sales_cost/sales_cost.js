...
function getFromFireBase1() {
    ...
    get(child(dbRef, `inventory_management/${gets.Username}`)).then((snapshot) => {
        if (snapshot.exists()) {
            if (snapshot.val().ledgers.eight != undefined) {
                extra.push(snapshot.val().ledgers.eight.AllEnteries)
            }
...
function factory(typeI, nameI) {
    ...
    for (let j = 0; j < getInfoDB[0].length; j++) {
        if (`expense ${nameI.toLowerCase()}` == getInfoDB[0][j].AccountName) {
            let x = Number(getInfoDB[0][j].Debit);
            x += Number(amount.value);
            getInfoDB[0][j].Debit = x;
            all_enteries.push({ "AccountName": `expense ${nameI.toLowerCase()}`, "Debit": amount.value, "Credit": 0, "typeOfAccount": "expense", "v": 0 , "date": current});
            break;
        }
        ff++;
    }
    if(ff == getInfoDB[0].length){
        getInfoDB[0].push({ "AccountName": `expense ${nameI.toLowerCase()}`, "Debit": amount.value, "Credit": 0, "typeOfAccount": "expense" })
        all_enteries.push({ "AccountName": `expense ${nameI.toLowerCase()}`, "Debit": amount.value, "Credit": 0, "typeOfAccount": "expense", "v": 0, "date": current });
    }
    let c = 0;
    ...
    accName.innerHTML = `expesne ${nameI}`;
...
function saveToFirebase(getUser) {
    ...
    get(child(dbRef, `inventory_management/${getUser.Username}`)).then((snapshot) => {
        // console.log("true")
        if (snapshot.exists()) {
            set(ref(db, "inventory_management/" + getUser.Username + `/ledgers/eight`), {
