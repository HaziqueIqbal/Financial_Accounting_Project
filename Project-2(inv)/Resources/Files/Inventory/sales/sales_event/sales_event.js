...
function getFromFireBase1() {
    ...
    get(child(dbRef, `inventory_management/${gets.Username}`)).then((snapshot) => {
        if (snapshot.exists()) {
            if (snapshot.val().ledgers.five != undefined) {
                extra.push(snapshot.val().ledgers.five.AllEnteries)
            }
            ...
            if (getInvName.value.toLowerCase() == getInfoDB[0][index].AccountName) {
                ...
                if (getInfoDB[0][index].priceDetails.length == undefined) {
                    ...
                    if (Number(noOfUnits.value) <= Number(getInfoDB[0][index].priceDetails.units)) {
                        ...
                        let z = x * y;
                        totalpayment = (z + (y * 5));
                        displayPrice.value = `Price for sale is ${totalpayment}`;
                    } else {
...
function factory(t, n, total, cogs) {
    ...
    for (let j = 0; j < getInfoDB[0].length; j++) {
        if (t == getInfoDB[0][j].AccountName) {
            ...
            all_enteries.push({ "AccountName": t, "Debit": Number(total), "Credit": 0, "typeOfAccount": "asset", "v": 0, "date": current });
            break;
        }
        accp++;
    }
    if (accp == getInfoDB[0].length) {
        getInfoDB[0].push({ "AccountName": "accounts receivable", "Debit": Number(total), "Credit": 0, "typeOfAccount": "asset" });
        all_enteries.push({ "AccountName": "accounts receivable", "Debit": Number(total), "Credit": 0, "typeOfAccount": "asset", "v": 0, "date": current });
    }
    let x = 0;
    for (let j = 0; j < getInfoDB[0].length; j++) {
        if ("sales revenue" == getInfoDB[0][j].AccountName) {
            ...
            all_enteries.push({ "AccountName": "sales revenue", "Debit": 0, "Credit": Number(total), "typeOfAccount": "revenue", "v": 1 });
            break;
        }
        x++;
    }
    if (x == getInfoDB[0].length) {
        getInfoDB[0].push({ "AccountName": "sales revenue", "Debit": 0, "Credit": Number(total), "typeOfAccount": "revenue" });
        all_enteries.push({ "AccountName": "sales revenue", "Debit": 0, "Credit": Number(total), "typeOfAccount": "revenue", "v": 1 });
    }
    ///
    ...
    for (let j = 0; j < getInfoDB[0].length; j++) {
        if (`cogs ${n}` == getInfoDB[0][j].AccountName) {
            ...
            getInfoDB[0][j].Debit = x;
            all_enteries.push({ "AccountName": `cogs ${n}`, "Debit": cogs * noOfUnits.value, "Credit": 0, "typeOfAccount": "expense", "v": 2 })
            break;
        }
        b++;
    }
    if (b == getInfoDB[0].length) {
        getInfoDB[0].push({ "AccountName": `cogs ${n}`, "Debit": cogs * noOfUnits.value, "Credit": 0, "typeOfAccount": "expense" });
        all_enteries.push({ "AccountName": `cogs ${n}`, "Debit": cogs * noOfUnits.value, "Credit": 0, "typeOfAccount": "expense", "v": 2 })
    }

    for (let m = 0; m < getInfoDB[0].length; m++) {
        if (n == getInfoDB[0][m].AccountName) {
            let x = Number(getInfoDB[0][m].Credit);
            x += Number(cogs * noOfUnits.value);
            getInfoDB[0][m].Credit = x;
            all_enteries.push({ "AccountName": `${n}`, "Debit": 0, "Credit": cogs * noOfUnits.value, "typeOfAccount": "asset", "v": 3 })
            if (getInfoDB[0][m].priceDetails.length == undefined) {
                ...
                let z = 0;
                let mmy = Number(noOfUnits.value);
                for (let k = 0; k < getInfoDB[0][m].priceDetails.length; k++) {
                    if (Number(getInfoDB[0][m].priceDetails[k].units) != 0) {
                        let bx = Number(getInfoDB[0][m].priceDetails[k].units);
                        if (mmy <= bx) {
                            bx -= mmy;
                            getInfoDB[0][m].priceDetails[k].units = bx;
                            break;
                        } else {
                            if (mmy == 0) {
                                break
                                ...
                                let temp = Number(getInfoDB[0][m].priceDetails[k].units);
                                mmy -= temp;
                                if (mmy != 0) {
                                    getInfoDB[0][m].priceDetails[k].units = 0;
                                } else {
                                    getInfoDB[0][m].priceDetails[k].units = Math.abs(mmy);
                                }
                            }
    ...
    deb2.innerHTML = cogs * noOfUnits.value;
    cre2.innerHTML = "";
    tr2.append(accName2);
    tr2.append(deb2);
    tr2.append(cre2);
    ...
    deb3.innerHTML = "";
    cre3.innerHTML = cogs * noOfUnits.value;
    tr3.append(accName3);
    tr3.append(deb3);
    tr3.append(cre3);
    GJ_Entry.append(tr3);
...
function saveToFirebase(getUser) {
    ...
    get(child(dbRef, `inventory_management/${getUser.Username}`)).then((snapshot) => {
        // console.log("true")
        if (snapshot.exists()) {
            set(ref(db, "inventory_management/" + getUser.Username + `/ledgers/five`), {
