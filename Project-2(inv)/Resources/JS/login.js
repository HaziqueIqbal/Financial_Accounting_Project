...
function Login(login_as) {
    ...
    const dbRef = ref(db);
    get(child(dbRef, `${login_as}/${fullName.value}`)).then((snapshot) => {
        if (snapshot.exists()) {
            ...
            if (password.value == dbPass) {
                ...
                fullName.value = "";
                password.value = "";
                if (la.value == "organization") {
                    window.location.replace("Files/Dashbord.html");
                } else {
                    window.location.replace("Files/Inventory_Dashboad.html");
                }
            }
            else {
